...
public class MailConfirmationConsommateur implements MessageListener {
	...
	public MailConfirmationConsommateur() throws Exception {
		...
		Connection conn = factory.createConnection("nasnas","nasty");
		// 3: création d’une session
		Session session = conn.createSession(false,Session.AUTO_ACKNOWLEDGE);
		// 4. Recherche d’une destination
		Topic topic = (Topic) ctx.lookup("jms/topic/MailConfirmationMdbTopic");
		...
		System.out.println("Client JMS MailConfirmationConsommateurà l’écoute de messages.");
		conn.start();
	}
